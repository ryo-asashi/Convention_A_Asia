...
In the `{midlearn}` library, the summary output includes this ratio calculated on the training set.
...
In the MID framework, the ICE for a feature $j$ and a specific observation $i$ is the expected value of the prediction as $X_j$ varies, while keeping other features fixed at their observed values $\mathbf{x}_{i,\setminus j}$:
...
![](../assets/Tree_Based_Model.png){fig-align="center" width="1000px"}
...
We use `{midlearn}` to replicate the LightGBM model.
...
A key advantage of `{midlearn}` is its ability to isolate interaction effects $g_{jk}$ from main effects $g_j$.
