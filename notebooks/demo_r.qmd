...
It deconstructs a black-box prediction function $f(\mathbf{X})$ into several interpretable components: intercept $g_\emptyset$, main effects $g_j(X_j)$, and interactions $g_{jk}(X_j, X_k)$, minimizing the squared residuals $\mathbf{E}\left[g_D(\mathbf{X})^2\right]$:
...
$$
\text{Loss}(\mathbf{y}, \hat{\mathbf{y}}, \mathbf{w}) =
\frac{\sum_{i=1}^n w_i\;d(y_i,\hat{y}_i)}{\sum_{i=1}^n w_i},\quad
d(y_i,\hat{y}_i) = 2\left[y_i (\log y_i - \log \hat{y}_i) - (y_i - \hat{y}_i)\right]
$$
...
We apply the `interpret()` function to the GAM to construct a first-order MID model—the sum of the intercept and main effects—as an interpretable surrogate of the original model.
...
```{r load_mid_lgb, echo = FALSE}
if (LOAD_MID_LGB) {
  mid_lgb <- qs::qread("../data/mid_lgb.qs")
} else {
  mid_lgb <- interpret(
    Frequency ~ (VehPower + VehAge + DrivAge + LogDensity +
                 VehBrand + VehGas + Region)^2,
...
if (SAVE_MID_LGB) {
  qs::qsave(mid_lgb, "../data/mid_lgb.qs")
}
summary(mid_lgb)
```
